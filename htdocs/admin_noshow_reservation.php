...
$resr_state = "노쇼";
  ...
  echo "<script>window.alert('" .$resr_no. "은 노쇼로 등록되었습니다.');</script>";
  echo "<script>location.href='admin_canceled.php';</script>";
  }

  $query = mysql_query("select * from reservation_table where user_no='" .$user_no. "' && resr_state='예약완료'") or die(mysql_error());
  $finished_rows = mysql_num_rows($query);
  $query1 = mysql_query("select * from reservation_table where user_no='" .$user_no. "' && resr_state='노쇼'") or die(mysql_error());
