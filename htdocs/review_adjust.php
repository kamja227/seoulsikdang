...
session_start();
   ...
   $connect = mysql_connect('localhost','root','950227');
    ...
    if($_SERVER["REQUEST_METHOD"] == "GET") {
      $rev_resr_no = mysql_real_escape_string($_GET["rev_resr_no"]);

      $query = "SELECT rev_resr_no, rev_rest_no, rev_user_no, rev_taste, rev_price, rev_mood, rev_kindness, rev_text FROM review_table WHERE rev_resr_no = '" .$rev_resr_no. "'";

      $result = mysql_query($query) or die(mysql_error());
      if(mysql_num_rows($result)){
       while($rows = mysql_fetch_assoc($result)){
          $rev_resr_no = $rows['rev_resr_no'];
          $rev_rest_no = $rows['rev_rest_no'];
  ...
  $query = mysql_query("SELECT resr_no, resr_rest_no, resr_date, resr_time FROM reservation_table WHERE resr_no = '" .$rev_resr_no. "'") or die(mysql_error());
...
<body>
    <div class='w3-half'>
    <div class='w3-container w3-white'>
    <h2 class='w3-border-bottom w3-border-light-grey w3-padding-16'>리뷰 수정</h2>
<div class='container'>
<form action='review_adjust_post.php' name='review' method='post'>
   <p><label>맛</label>
    <input type='text' class='rating rating-loading' value='" .$rev_taste. "' name='rev_taste' data-min='0' data-max='5'  data-step='0.5' data-size='xs'>
   <p><label>가격</label>
 <input type='text' class='rating rating-loading' value='" .$rev_price. "' name='rev_price' data-min='0' data-max='5'  data-step='0.5' data-size='xs'>   
  <p><label>분위기</label>
    <input type='text' class='rating rating-loading' value='" .$rev_mood. "' name='rev_mood' data-min='0' data-max='5'  data-step='0.5' data-size='xs'>
   <p><label>친절함</label>
  <input type='text' class='rating rating-loading' value='" .$rev_kindness. "' name='rev_kindness' data-min='0' data-max='5'  data-step='0.5' data-size='xs'>
   <p><label>고객 후기</label></p>
   <p>
 <textarea name='rev_text' cols='80' rows='7' onkeyup='fc_chk_byte(this, 500)' placeholder='".$rev_text. "'></textarea></p>
    <input type='hidden' name='rev_resr_no' value='" .$rev_resr_no. "'>
    <input type='hidden' name='rev_rest_no' value='" .$resr_rest_no. "'> 
    <input type='submit' class='w3-button w3-red w3-margin-top' value='수정'>
