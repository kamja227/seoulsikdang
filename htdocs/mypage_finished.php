...
<?php 
     ...
     while($rows = mysql_fetch_array($query)){
        ...
        $query0 = mysql_query ("SELECT rev_no FROM review_table WHERE rev_resr_no ='" .$resr_no."'")or die(mysql_error());
        $numrow = mysql_num_rows($query0);
        if($numrow > 0) { 
          echo "<a href='review_adjust.php?rev_resr_no=" .$resr_no. "' class='w3-button w3-margin-top w3-blue'>리뷰 수정</a>
                <a href='review_delete.php?rev_resr_no=" .$resr_no. "' class='w3-button w3-margin-top w3-red'>리뷰 삭제</a>"; }
           
        else 
        echo "<a href='review.php?resr_no=" .$resr_no. "' class='w3-button w3-margin-top w3-blue' > 리뷰 작성</a>"; 
