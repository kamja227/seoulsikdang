...
/*  $query = mysql_query("select resr_no, resr_date, resr_time, resr_total, reserver_name from reservation_table where resr_user_no='" .$_SESSION['user_no']. "'") or die(mysql_error());
  ...
  $query5=mysql_query("select user_name, user_grade from customer_table where user_no ='" .$user_no."'")or die(mysql_error());
      if(mysql_num_rows($query5)){
      while($rows = mysql_fetch_assoc($query5)){
...
if ($user_grade=="bronze") {
  echo "<img src= 'bronze.png' width='30px' height='30x' class='w3-hover-opacity'>";
} 
elseif($user_grade=="silver"){
  echo "<img src= 'silver.png' width='30px' height='30x' class='w3-hover-opacity'>";
}
else {
  echo "<img src= 'gold.png' width='30px' height='30x' class='w3-hover-opacity'>";
}
echo" 등급 입니다.     ";
...
<?php 
     $conn = mysql_connect('localhost','root','0920');
     $db_status = mysql_select_db('reservation2',$conn);
     if (!$db_status)
     {
       echo("DB_ERROR");
     ...
     $query = mysql_query("SELECT resr_no,resr_rest_no, resr_date, resr_time, resr_total, resr_state, reserver_name, reserver_tel, reserver_email, resr_requirement FROM reservation_table WHERE resr_user_no = '" .$_SESSION['user_no']. "'") or die(mysql_error());
     while($rows = mysql_fetch_array($query)){
        ...
        <div class='w3-button w3-red w3-margin-top'><a href='mypage_canceled_reservation.php?resr_no=" .$resr_no. "'>예약 취소 </a></div>
         ";
